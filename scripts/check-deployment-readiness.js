...
function checkScripts(packageJson) {
  ...
  if (missing.length === 0) {
    checks.push({
      status: "pass",
      message: "Required scripts (build, dev) exist",
    });
  } else {
...
function checkEnvDocs() {
  ...
  if (fs.existsSync(readmePath)) {
    ...
    if (hasEnvDocs) {
      checks.push({
        status: "pass",
        message: "Environment variables documented in README",
      });
    } else {
...
function checkBuildIssues(packageJson) {
  ...
  if (packageJson.engines?.node) {
    ...
    warnings.push(
      "Node.js version not specified in package.json (engines.node)",
    );
  }
}
...
function checkEnvExample() {
  ...
  if (fs.existsSync(envLocalPath)) {
    warnings.push(
      ".env.local exists (make sure to set env vars in Vercel dashboard, not just locally)",
    );
  }
}
...
function main() {
  ...
  if (errors.length > 0) {
    ...
    log(
      "\n⚠️  Review the warnings above. They may cause deployment issues.",
      "yellow",
    );
  } else {
