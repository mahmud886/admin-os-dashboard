...
const userEmail =
  env.NEXT_PUBLIC_STATIC_ADMIN_EMAIL || "iqbal886mahmud@gmail.com";
...
if (!serviceRoleKey) {
  ...
  console.error(
    '   3. Copy the service_role key (it starts with "eyJ..." or similar)',
  );
      ...
      res.on("end", () => {
        try {
          ...
          if (res.statusCode === 200 || res.statusCode === 201) {
            ...
            console.log(
              `   Confirmed: ${response.email_confirmed_at ? "Yes" : "No"}`,
            );
            resolve(response);
          } else if (
            res.statusCode === 422 &&
            ...
            console.log(
              "   1. Verify the password in Supabase Dashboard matches",
            );
            ...
            console.error(
              "❌ ERROR:",
              response.message || response.error_description || data,
            );
