...
export default function SupporterDonationsPage() {
  ...
  const handleExportCSV = () => {
    if (!donations.length) {
      addToast({ title: 'No data', description: 'No donations to export', variant: 'warning' });
      return;
    }
    ...
    addToast({ title: 'Exported', description: 'Donations list exported to CSV', variant: 'success' });
  };
    ...
    <MainLayout breadcrumb="SYSTEM CONSOLE / SUPPORTER DONATIONS">
      <div className="space-y-6">
        <div className="flex flex-col gap-4 justify-between items-start sm:flex-row sm:items-center">
          <h1 className="text-2xl font-bold text-teal-400 sm:text-3xl lg:text-4xl">SUPPORTER DONATIONS</h1>
          ...
          <CardContent className="p-6">
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <div className="p-4 rounded border border-teal-400/50 bg-teal-900/20">
                ...
                <div className="text-sm text-gray-300">$ {tierPrices['THE ARCHIVIST'].USD}</div>
                <div className="text-xs text-gray-400">USERS: {stats.archivist}</div>
              </div>
              <div className="p-4 rounded border border-teal-400/50 bg-teal-900/20">
                <div className="font-semibold text-teal-400">THE EMBLEM</div>
                <div className="text-sm text-gray-300">$ {tierPrices['THE EMBLEM'].USD}</div>
                <div className="text-xs text-gray-400">USERS: {stats.emblem}</div>
              </div>
              <div className="p-4 rounded border border-teal-400/50 bg-teal-900/20">
                <div className="font-semibold text-teal-400">THE PATRON</div>
                <div className="text-sm text-gray-300">$ {tierPrices['THE PATRON'].USD}</div>
                <div className="text-xs text-gray-400">USERS: {stats.patron}</div>
              </div>
              <div className="flex flex-col gap-2 p-4 rounded border border-yellow-400/50 bg-yellow-900/20">
                <div className="font-semibold text-yellow-400">SUPPORT THE UNIVERSE</div>
                <div className="text-sm text-gray-300">DONATE ANY AMOUNT</div>
                <div className="flex justify-between items-center">
                  <div className="text-xs text-gray-400">USERS: {stats.donation}</div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-border">
                    ...
                    <th className="p-4 text-sm font-medium text-left text-gray-400">ADDRESS</th>
                    <th className="p-4 text-sm font-medium text-left text-gray-400">AMOUNT</th>
                    <th className="p-4 text-sm font-medium text-left text-gray-400">TYPE</th>
                    <th className="p-4 text-sm font-medium text-left text-gray-400">TIER</th>
                    <th className="p-4 text-sm font-medium text-left text-gray-400">STATUS</th>
                    <th className="p-4 text-sm font-medium text-left text-gray-400">CREATED</th>
                  </tr>
                </thead>
                  ...
                  {loading ? (
                    ...
                    donations.map((p, i) => (
                      <tr key={p.id} className="hover:bg-accent/5">
                        ...
                        <td className="p-4 font-medium text-teal-400">{p.supporter_name || 'Anonymous'}</td>
                        <td className="p-4 text-gray-300">{p.supporter_email}</td>
                        <td className="p-4 max-w-xs text-gray-300 truncate" title={p.mailing_address}>
                          {p.mailing_address || '—'}
                        </td>
