...
const calculateTotal = (order) => {
  ...
  const baseAmount =
    typeof order.amount_total === "string"
      ? parseFloat(order.amount_total)
      : Number(order.amount_total) || 0;
  const tax =
    typeof order.metadata?.tax === "string"
      ? parseFloat(order.metadata.tax)
      : Number(order.metadata?.tax) || 0;
  const shipping =
...
export default function OrdersPage() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(
        `/api/ecommerce/orders/${orderToDelete.id}`,
        {
          method: "DELETE",
        },
      );
  ...
  const handleExportOrdersCSV = () => {
    ...
    addToast({
      title: "Exported",
      description: "Orders list exported to CSV",
      variant: "success",
    });
  };
                          ...
                          >
                            ...
                            <td className="p-4">
                              ...
                              <div className="text-xs text-gray-500">
                                {order.ecommerce_customers?.email}
                              </div>
                              <div className="text-xs text-gray-500">
                                {order.ecommerce_customers?.phone}
                              </div>
                            </td>
                            <td className="p-4">
                              <Badge
          ...
          <DialogContent className="bg-[#111111] border-border text-white max-w-2xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription className="text-gray-400">
                ...
                {selectedOrder?.ecommerce_order_items?.length || 0} Items
              </DialogDescription>
            </DialogHeader>
            ...
            {selectedOrder && (
              <div className="mt-4 space-y-6">
                ...
                <div className="flex flex-wrap gap-4 p-4 bg-[#0a0a0a] rounded-lg border border-border">
                  ...
                  <div className="flex flex-col gap-1 ml-auto text-right">
                      ...
                      <span className="text-white">
                        {formatCurrency(
                          selectedOrder.subtotal ||
                            selectedOrder.metadata?.subtotal ||
                            0,
                      ...
                      <span className="text-white">
                        {formatCurrency(
                          selectedOrder.shipping ||
                            selectedOrder.metadata?.shipping ||
                            0,
                          selectedOrder.currency,
                        )}
                      </span>
                    ...
                    <div className="text-xs text-gray-500">
                      ...
                      <span className="text-white">
                        {formatCurrency(
                          selectedOrder.tax || selectedOrder.metadata?.tax || 0,
                          selectedOrder.currency,
                        )}
                      </span>
                    </div>
                  ...
                  <div className="p-4 bg-[#0a0a0a] rounded-lg border border-border">
                    ...
                    <div className="space-y-1 text-sm">
                      <p className="font-medium text-white">
                        {selectedOrder.ecommerce_customers?.name || "Guest"}
                      </p>
                      <p className="text-gray-400">
                        {selectedOrder.ecommerce_customers?.email}
                      </p>
                      <p className="text-gray-400">
                        {selectedOrder.ecommerce_customers?.phone}
                      </p>
                    </div>
                  </div>
                  <div className="p-4 bg-[#0a0a0a] rounded-lg border border-border">
                ...
                {/* Order Items */}
                <div>
                  <h3 className="mb-3 text-sm font-medium text-teal-400">
                    ORDER ITEMS
                  </h3>
