...
export async function PUT(request, { params }) {
  try {
    ...
    if (fetchError) {
      if (fetchError.code === "PGRST116") {
        return createErrorResponse(
          "Episode not found",
          404,
          `Episode with ID ${id} does not exist`,
        );
      }
      return createErrorResponse(
    ...
    if (body.episode_number !== undefined)
      updateData.episode_number = parseInt(body.episode_number);
    if (body.season_number !== undefined)
      updateData.season_number = parseInt(body.season_number);
    if (body.runtime !== undefined) updateData.runtime = body.runtime;
    if (body.unique_episode_id !== undefined)
      updateData.unique_episode_id = body.unique_episode_id;
    if (body.visibility !== undefined) updateData.visibility = body.visibility;
    if (body.access_level !== undefined)
      updateData.access_level = body.access_level;
    if (body.release_datetime !== undefined)
      updateData.release_datetime = body.release_datetime || null;
    if (body.clearance_level !== undefined)
      updateData.clearance_level = parseInt(body.clearance_level);
    if (body.notify !== undefined) updateData.notify = body.notify;
    if (body.age_restricted !== undefined)
      updateData.age_restricted = body.age_restricted;
    if (body.thumb_image_url !== undefined)
      updateData.thumb_image_url = body.thumb_image_url;
    if (body.banner_image_url !== undefined)
      updateData.banner_image_url = body.banner_image_url;
    ...
    if (body.additional_background_image_url !== undefined)
      updateData.additional_background_image_url =
        body.additional_background_image_url;
    if (body.tags !== undefined)
      updateData.tags = Array.isArray(body.tags)
        ? body.tags.filter((tag) => tag.trim() !== "")
        : [];
    if (body.primary_genre !== undefined)
      updateData.primary_genre = body.primary_genre;
    if (body.secondary_genre !== undefined)
      updateData.secondary_genre = body.secondary_genre;
    if (body.status !== undefined || body.isDraft !== undefined) {
      updateData.status =
        body.status || (body.isDraft === false ? "PUBLISHED" : "DRAFT");
    }
