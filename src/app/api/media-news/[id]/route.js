...
export async function GET(request, { params }) {
  ...
  if (!article) {
    return NextResponse.json({ error: 'Media news not found' }, { status: 404 });
  }
...
export async function PUT(request, { params }) {
  try {
    ...
    return NextResponse.json({ error: 'Failed to update media news' }, { status: 500 });
  }
}
...
export async function DELETE(request, { params }) {
  ...
  if (!success) {
    return NextResponse.json({ error: 'Failed to delete media news' }, { status: 500 });
  }
