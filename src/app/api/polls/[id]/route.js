  ...
  try {
    ...
    if (!isVotingRequest) {
      ...
      if (authError || !user) {
        return createErrorResponse(
          "Unauthorized",
          401,
          "Authentication required for admin updates",
        );
      }
    }
    ...
    if (isVotingRequest) {
      ...
      if (existing.status !== "LIVE") {
        return createErrorResponse(
          ...
          400,
          `Poll status is ${existing.status}`,
        );
      }
    ...
    if (body.duration_days !== undefined) {
      ...
      if (!body.ends_at) {
        ...
        const calculatedEndDate = calculatePollEndDate(
          startsAt,
          body.duration_days,
        );
    ...
    if (body.status !== undefined || body.isDraft !== undefined) {
      updateData.status =
        body.status || (body.isDraft === false ? "LIVE" : "DRAFT");
    }
